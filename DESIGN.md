...
### 3.2 Components
- **`qm` (CLI):** A lightweight command-line tool for user interaction (e.g., `qm up`, `qm status`).
...
- **Network Manager:** Man[]ages network profiles, including a "sidecar" approach for VPN-based routing.
...
- Implement Network Profiles and CNI-based routing (VPN sidecars).
