	...
	return &Checker{
		httpClient: &http.Client{
			...
			CheckRedirect: func(req *http.Request, via []*http.Request) error {
				return http.ErrUseLastResponse // don't follow — check the actual endpoint
			},
		},
	}
}

// RunCheck executes a single health check against a container.
// bridgeIP is the bridge IP for non-host-networked containers;
// pass "" for host-networked containers (probes localhost).
func (c *Checker) RunCheck(svc types.Service, bridgeIP string) Result {
	hc := svc.HealthCheck
	start := time.Now()
...
func (c *Checker) checkHTTP(svc types.Service, bridgeIP string) error {
	...
	url := fmt.Sprintf("http://%s:%d%s", host, port, hc.Path)
...
// Uses localhost for all probes (host-networked containers).
