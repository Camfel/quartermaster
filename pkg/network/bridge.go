...
// Bridge IPs are /24 (254 usable), minus the gateway (.1) = 253 allocatable.
func (b *BridgeManager) IPCount() int {
	b.mu.Lock()
	defer b.mu.Unlock()
	return len(b.ips)
}
...
func (b *BridgeManager) IPFree() int {
	b.mu.Lock()
	defer b.mu.Unlock()
	const maxUsable = 253
	free := maxUsable - len(b.ips)
	if free < 0 {
		return 0
	}
	return free
...
func (b *BridgeManager) tryConfigureGateway(serviceName string) bool {
	...
	// Lock the OS thread so Go's scheduler doesn't leak the namespace
	// switch to other goroutines (AGENTS.md rule #1).
	runtime.LockOSThread()
