...
type MockContainerClient struct {
	...
	// OnContainerStats is called when ContainerStats is invoked.
	OnContainerStats func(containerID string) (*ContainerStats, error)
}
...
// ContainerStats implements the ContainerClient interface.
func (m *MockContainerClient) ContainerStats(ctx context.Context, containerID string) (*ContainerStats, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	if m.OnContainerStats != nil {
		return m.OnContainerStats(containerID)
	}
