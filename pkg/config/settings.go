...
type Settings struct {
	...
	ComponentToken string           `json:"component_token,omitempty"`

	// MetricsSettings controls the Prometheus-compatible metrics endpoint.
...
type MetricsSettings struct {
	// Enabled starts a TCP listener for VictoriaMetrics/Prometheus scraping.
	// The /v1/metrics endpoint on the Unix socket is always available.
...
// Both fields must be set for alerting to be active.
type AlertingSettings struct {
	// GotifyURL is the base URL of the Gotify server (e.g. "http://gotify:80").
	// Leave empty to disable alerting.
	GotifyURL string `json:"gotify_url"`

	// GotifyToken is the app token created in the Gotify web UI.
...
func (s *Settings) applyDefaults() {
	...
	if s.ComponentsRepo == "" {
		s.ComponentsRepo = "https://github.com/quartermaster-hq/qm-components.git"
	}

	// ── Metrics defaults ───────────────────────────────────────
	if s.Metrics.ListenAddr == "" {
		s.Metrics.Enabled = true
		s.Metrics.ListenAddr = "127.0.0.1:9098"
	}
...
func (s *Settings) Validate() error {
	...
	// ── Metrics validation ────────────────────────────────────
	if s.Metrics.Enabled && s.Metrics.ListenAddr != "" {
		host, _, err := net.SplitHostPort(s.Metrics.ListenAddr)
		if err != nil {
			return fmt.Errorf("metrics.listen_addr: invalid address %q: %w", s.Metrics.ListenAddr, err)
		}
		if host != "127.0.0.1" && host != "::1" && host != "localhost" {
			return fmt.Errorf("metrics.listen_addr must be a loopback address (127.0.0.1, ::1, or localhost), got %q", s.Metrics.ListenAddr)
		}
	}
